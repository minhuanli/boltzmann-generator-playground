...
class InvNet(object):
    ...
    def log_likelihood_z_lognormal(self, std=1.0):
        """ Returns the log likelihood of z|x assuming a Log Normal distribution in z
        """
    ...
    def rc_entropy(self, rc_func, gmeans, gstd, ntemperatures=1):
        ...
        ----------
        rc_func : function
          function to compute reaction coordinates
        gmeans : array
          mean positions of Gauss kernels along reaction coordinates
        gstd : float
          standard deviation of Gauss kernels along reaction coordinates
        """

        # evaluate rc
        rc = rc_func(self.output_x)
        rc = tf.expand_dims(rc, axis=-1)
        gmeans = tf.expand_dims(gmeans, axis=0)
        gstd = tf.expand_dims(gstd, axis=-1)

        # kernelize all values
        w_delta_sq_sum = tf.reduce_sum((rc - gmeans)**2./(2*gstd*gstd), axis=1)
        kmat = tf.exp(-w_delta_sq_sum)

        # prior pseudo density
        kmat += 1e-8

        # normalize over kernels
        kmat /= tf.reduce_sum(kmat, axis=1, keepdims=True)
        batch_size = tf.shape(kmat)[0]
        nbins = tf.shape(kmat)[1]

        # distribute counts across temperatures
        batchsize_per_temperature = tf.cast(
            batch_size // ntemperatures, tf.int32)
    ...
    def rc_entropy_old(self, rc_func, gmeans, gstd, ntemperatures=1):
        """ Computes the entropy along a 1D reaction coordinate
        Parameters
        ----------
        rc_func : function
        ...
        # distribute counts across temperatures
        batchsize_per_temperature = tf.cast(
            tf.shape(kmat)[0] / ntemperatures, tf.int32)
    ...
    def reg_Jzx_uniform(self):
        """ Returns the log likelihood of z|x assuming a Uniform distribution in z
        """
        # return self.log_det_Jxz - self.dim * tf.log(std) - (0.5 / (std**2)) * tf.reduce_sum(self.output_z**2, axis=1)
        Jmean = tf.reduce_mean(self.log_det_Jzx, axis=0, keepdims=True)
        Jdev = tf.reduce_mean((self.log_det_Jzx - Jmean)
                              ** 2, axis=1, keepdims=False)
        return Jdev
    ...
    def reg_Jxz_uniform(self):
        ...
        Jdev = tf.reduce_mean((self.log_det_Jxz - Jmean)
                              ** 2, axis=1, keepdims=False)
        return Jdev
    ...
    def log_likelihood_z_normal_2trajs(self, trajlength, std=1.0):
        ...
        J = self.log_det_Jxz
        LL1 = tf.reduce_mean(J[:trajlength] - (0.5 / (std**2))
                             * tf.reduce_sum(self.output_z[:trajlength]**2, axis=1))
        LL2 = tf.reduce_mean(J[trajlength:] - (0.5 / (std**2))
                             * tf.reduce_sum(self.output_z[trajlength:]**2, axis=1))
...
class EnergyInvNet(InvNet):
    ...
    def train_KL(self, optimizer=None, lr=0.001, epochs=2000, batch_size=1024, verbose=1, clipnorm=None,
        ...
        tfac = np.tile(temperature, int(
            batch_size / temperature.size) + 1)[:batch_size]
        ...
        for e in range(epochs):
            ...
            w = self.sample_z(
                temperature=tfac[:, None], nsample=batch_size, return_energy=False)
    ...
    def train_flexible(self, x, xval=None, optimizer=None, lr=0.001, epochs=2000, batch_size=1024, verbose=1,
        ...
        def loss_MCEff_combined(y_true, y_pred):
            return -self.log_GaussianPriorMCMC_efficiency(high_energy, max_energy, metric=metric, symmetric=symmetric_MC) \
                - 0.5 * self.log_GaussianPriorMCMC_efficiency_unsupervised(
                    high_energy, max_energy, metric=metric)
        ...
        dual_model.compile(optimizer=optimizer, loss=losses,
        ...
        for e in range(epochs):
            ...
            w_batch = np.sqrt(tfac)[:, None] * \
                np.random.randn(batch_size, self.dim)
            l = dual_model.train_on_batch(x=[x_batch, w_batch], y=y)
            loss_train.append(l)
            # validate
            if xval is not None:
                xval_batch = xval[np.random.choice(
                    I, size=batch_size, replace=True)]
                wval_batch = np.sqrt(tfac)[:, None] * \
                    np.random.randn(batch_size, self.dim)
