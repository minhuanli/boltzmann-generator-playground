...
class InvNet(object):
    ...
    def rc_entropy(self, rc_func, gmeans, gstd, ntemperatures=1):
        ...
        batchsize_per_temperature = tf.cast(batch_size // ntemperatures, tf.int32)
        ...
        if ltype == '<':
           ...
           if permute_atomwise:
            ...
            if ic_cart is None:
                ...
                layer = MixedCoordinatesTransformation(ic_cart, ic, X0=ic_norm, torsion_cut=torsion_cut, jacobian_regularizer=ic_jacobian_regularizer)
            layers.append(layer)
        elif ltype == 'W':
            if dim_R > 0:
