...
class MM(object):
    ...
    def bondlengths(self, x):
        dists = dist(x[:, self.atom_indices[self.toppar.bond_indices[:, 0]]],
                     x[:, self.atom_indices[self.toppar.bond_indices[:, 1]]])
        return dists

    def nb_dists(self, x):
        dists = dist(x[:, self.atom_indices[self.toppar.nonbonded_indices[:, 0]]],
                     x[:, self.atom_indices[self.toppar.nonbonded_indices[:, 1]]])
        return dists

    def nb14_dists(self, x):
        dists = dist(x[:, self.atom_indices[self.toppar.nonbonded14_indices[:, 0]]],
                     x[:, self.atom_indices[self.toppar.nonbonded14_indices[:, 1]]])
        return dists

    def angles(self, x):
        a = angle(x[:, self.atom_indices[self.toppar.angle_indices[:, 0]]],
                  x[:, self.atom_indices[self.toppar.angle_indices[:, 1]]],
                  x[:, self.atom_indices[self.toppar.angle_indices[:, 2]]])
        return a

    def torsions(self, x):
        t = torsion(x[:, self.atom_indices[self.toppar.torsion_indices[:, 0]]],
    ...
    def bondlengths_tf(self, x):
        dists = dist_tf(tf.gather(x, self.atom_indices[self.toppar.bond_indices[:, 0]], axis=1),
                        tf.gather(x, self.atom_indices[self.toppar.bond_indices[:, 1]], axis=1))
        return dists

    def nb_dists_tf(self, x):
        dists = dist_tf(tf.gather(x, self.atom_indices[self.toppar.nonbonded_indices[:, 0]], axis=1),
                        tf.gather(x, self.atom_indices[self.toppar.nonbonded_indices[:, 1]], axis=1))
        return dists

    def nb14_dists_tf(self, x):
        dists = dist_tf(tf.gather(x, self.atom_indices[self.toppar.nonbonded14_indices[:, 0]], axis=1),
                        tf.gather(x, self.atom_indices[self.toppar.nonbonded14_indices[:, 1]], axis=1))
        return dists

    def angles_tf(self, x):
        a = angle_tf(tf.gather(x, self.atom_indices[self.toppar.angle_indices[:, 0]], axis=1),
    ...
    def torsions_tf(self, x):
        t = torsion_tf(tf.gather(x, self.atom_indices[self.toppar.torsion_indices[:, 0]], axis=1),
...
class Amber(object):
    def __init__(self):
        ...
        self.torsion_pars = [['*', 'CT', 'CT', '*', 1.45 / (0.6*9.0), 0, 3],
                             ...
                             ['*', 'CH3', 'CT', '*', 1.45 / (0.6*9.0), 0, 3],
                             ['*', 'CH3', 'CH', '*', 1.45 / (0.6*9.0), 0, 3],
                             ['*', 'CH3', 'CH3', '*', 1.45 / (0.6*9.0), 0, 3],
                             ]
